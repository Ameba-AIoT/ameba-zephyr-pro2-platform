 ...
 */
#define DT_DRV_COMPAT realtek_amebapro2_flash_controller
...
static int flash_ameba_read(const struct device *dev, off_t address, void *buffer, size_t length)
{
	...
	hal_flash_stream_read(&hal_spic_adaptor, length, address, (u8 *)buffer);
							 ...
							 size_t length)
{
	...
	hal_flash_burst_write(&hal_spic_adaptor, length, address, (u8 *)buffer);
...
static int flash_ameba_erase(const struct device *dev, off_t offset, size_t len)
{
	...
	for (i = start_sector_offset; i <= end_sector_offset; i++) {
		hal_flash_sector_erase(&hal_spic_adaptor, start_sector_offset);
	}
